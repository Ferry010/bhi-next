...
export default function OriginTour() {
  ...
  return (
    <div
      ref={containerRef}
      className="h-[100dvh] overflow-y-scroll snap-y snap-proximity md:snap-mandatory scroll-smooth bg-near-black text-white"
    >
      {/* Dot navigation */}
      <nav
        ...
        {Array.from({ length: SECTION_COUNT }).map((_, i) => (
          <button
            ...
            className={`group relative w-2.5 h-2.5 rounded-full transition-all duration-300 ${
              active === i ? "bg-sunny scale-125" : "bg-white/25 hover:bg-white/60"
            }`}
          >
            <span className="pointer-events-none absolute right-6 top-1/2 -translate-y-1/2 whitespace-nowrap text-xs font-heading font-semibold text-white/70 opacity-0 group-hover:opacity-100 transition-opacity">
              {labels[i]}
            </span>
          </button>
        ))}
      ...
      <section
        data-index={0}
        ref={setRef(0)}
        className="relative h-[100dvh] snap-start snap-always flex items-center justify-center overflow-hidden px-6"
      >
        <div className="absolute -top-24 left-1/2 -translate-x-1/2 w-[42rem] h-[42rem] rounded-full bg-primary/25 blur-[130px]" />
        <div className="relative text-center max-w-3xl">
          <span className="text-sunny text-caption uppercase tracking-[0.25em] font-heading font-semibold">Our story</span>
          <h1 className="text-hero md:text-hero-lg font-heading font-extrabold mt-5 leading-[1.05]">
            It started with a line <br className="hidden md:block" />in a notebook.
          </h1>
          <p className="text-white/70 text-base md:text-body-lg mt-6 max-w-xl mx-auto">
            How two growth hackers turned their own discomfort into Brand Humanizing, and where it goes next.
          </p>
          <button
            onClick={() => go(1)}
            className="mt-12 inline-flex flex-col items-center gap-2 text-white/60 hover:text-white transition-colors"
          >
      ...
      <section
        data-index={1}
        ref={setRef(1)}
        className="relative min-h-[100dvh] md:h-[100dvh] snap-start snap-always flex flex-col items-center justify-start md:justify-center overflow-hidden px-6 pt-24 pb-16 md:py-24"
      >
        <div className="relative w-full max-w-6xl grid md:grid-cols-2 gap-6 md:gap-14 items-center">
          <div>
            <span className="text-sunny text-caption uppercase tracking-widest font-heading font-semibold">2017 · Rotterdam</span>
            <h2 className="text-display md:text-display-lg font-heading font-extrabold mt-3 leading-tight">Where it all began</h2>
            <p className="text-white/60 font-heading font-semibold mt-2">A McDonald&apos;s in Rotterdam.</p>
            <div className="mt-5 space-y-3 text-sm md:text-base text-white/70 leading-relaxed max-w-xl">
              <p>
            ...
            <blockquote className="mt-6 border-l-2 border-accent pl-4 text-xl md:text-2xl font-heading font-bold text-white leading-snug max-w-md">
              &ldquo;Don&apos;t automate for humans. Automate for processes.&rdquo;
            </blockquote>
            <p className="text-sm text-white/50 mt-5 max-w-xl">
      ...
      <section
        data-index={2}
        ref={setRef(2)}
        className="relative min-h-[100dvh] md:h-[100dvh] snap-start snap-always flex flex-col items-center justify-start md:justify-center overflow-hidden px-6 pt-24 pb-16 md:py-24 bg-navy"
      >
        <div className="relative w-full max-w-6xl grid md:grid-cols-2 gap-6 md:gap-14 items-center">
          ...
          <div className="order-1 md:order-2">
            <span className="text-sunny text-caption uppercase tracking-widest font-heading font-semibold">2018 · First stages</span>
            <h2 className="text-display md:text-display-lg font-heading font-extrabold mt-3 leading-tight">Saying it out loud</h2>
            <p className="text-white/60 font-heading font-semibold mt-2">The first time we used the words in public.</p>
            <div className="mt-5 space-y-3 text-sm md:text-base text-white/70 leading-relaxed max-w-xl">
              <p>
      ...
      >
        <div className="relative w-full max-w-6xl grid md:grid-cols-2 gap-6 md:gap-14 items-center">
          <div>
            <span className="text-sunny text-caption uppercase tracking-widest font-heading font-semibold">The first believer</span>
            <h2 className="text-display md:text-display-lg font-heading font-extrabold mt-3 leading-tight">Our first client</h2>
            <p className="text-white/60 font-heading font-semibold mt-2">ABN AMRO turned a talk into a project.</p>
            <div className="mt-5 space-y-3 text-sm md:text-base text-white/70 leading-relaxed max-w-xl">
              <p>
          ...
          <div className="flex justify-center md:justify-end">
            ...
            <figure className="bg-white px-5 pt-8 pb-3 shadow-[0_18px_50px_-12px_rgba(0,0,0,0.65)] ring-1 ring-black/5 w-64 md:w-72 -rotate-1">
      ...
      >
        <div className="relative w-full max-w-6xl grid md:grid-cols-2 gap-6 md:gap-14 items-center">
          ...
          <div className="order-1 md:order-2">
            <span className="text-sunny text-caption uppercase tracking-widest font-heading font-semibold">2021 → today</span>
            <h2 className="text-display md:text-display-lg font-heading font-extrabold mt-3 leading-tight">From an idea to the world&apos;s stages</h2>
            <p className="text-white/60 font-heading font-semibold mt-2">Forty keynotes a year, and counting.</p>
            <p className="mt-5 text-sm md:text-base text-white/70 leading-relaxed max-w-xl">
            ...
            <div className="flex flex-wrap gap-x-8 gap-y-3 mt-7">
              {[
                ...
                <div key={s.l}>
                  <div className="font-heading font-extrabold text-2xl md:text-3xl text-sunny">{s.v}</div>
                  <div className="text-white/50 text-xs md:text-sm">{s.l}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      >
        <div className="relative w-full max-w-6xl grid md:grid-cols-2 gap-6 md:gap-14 items-center">
          <div>
            <span className="text-sunny text-caption uppercase tracking-widest font-heading font-semibold">November 2025</span>
            <h2 className="text-display md:text-display-lg font-heading font-extrabold mt-3 leading-tight">We wrote it all down</h2>
            <p className="text-white/60 font-heading font-semibold mt-2">Brand Humanizing, the book.</p>
            <div className="mt-5 space-y-3 text-sm md:text-base text-white/70 leading-relaxed max-w-xl">
              <p>
      ...
      <section
        data-index={6}
        ref={setRef(6)}
        className="relative h-[100dvh] snap-start snap-always flex flex-col items-center justify-center overflow-hidden px-6 text-center"
      >
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[42rem] h-[42rem] rounded-full bg-accent/20 blur-[130px]" />
        <div className="relative max-w-2xl">
          <span className="text-sunny text-caption uppercase tracking-widest font-heading font-semibold">Where it&apos;s going</span>
          <h2 className="text-display md:text-display-lg font-heading font-extrabold mt-4 leading-tight">The next chapter is your team.</h2>
          <p className="text-white/70 text-base md:text-body-lg mt-6 max-w-xl mx-auto">
        ...
        <div className="absolute bottom-6 left-0 right-0 flex items-center justify-center gap-3 text-xs text-white/30">
          <Link href="/" className="hover:text-white/60 transition-colors">Brand Humanizing Institute</Link>
          <span>·</span>
          <Link href="/our-beliefs" className="hover:text-white/60 transition-colors">Our beliefs</Link>
          <span>·</span>
          <Link href="/the-method" className="hover:text-white/60 transition-colors">The method</Link>
        </div>
      </section>
    </div>
  );
