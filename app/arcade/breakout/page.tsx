...
export default function BreakoutPage() {
      ...
      for (const ball of gs.balls) {
        ...
        for (const b of gs.bricks) {
          ...
          if (
            ...
            if (min === ot || min === ob) ball.dy = -ball.dy; else ball.dx = -ball.dx;
          }
        }
      }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-16" style={{ background: '#0a0a0f' }}>
      ...
      <div className="mb-6">
        <span
          className="inline-block px-3 py-1 rounded-full text-[11px] font-heading font-semibold tracking-[0.18em] uppercase"
          style={{ background: 'rgba(155,63,245,0.15)', color: '#9B3FF5', border: '1px solid rgba(155,63,245,0.35)' }}
        >
          Archived — 404 Game #1
        </span>
      </div>
      ...
      <h1
        className="font-heading font-black leading-none tracking-tighter select-none text-center"
        style={{
      ...
      <p className="font-body text-center mt-3 mb-8" style={{ color: 'rgba(255,255,255,0.55)', fontSize: '15px', maxWidth: '420px' }}>
        Classic brick-breaker with five levels, four powerups, and one life.
        The original game that lived on our 404 page.
      </p>

      <div className="flex gap-3 flex-wrap justify-center">
        <Link
          href="/arcade"
          className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-heading font-semibold text-white text-[15px] transition-opacity hover:opacity-90 active:scale-[0.97]"
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-heading font-semibold text-[15px] transition-all hover:bg-white/5 active:scale-[0.97]"
          style={{ color: '#E040C8', border: '2px solid rgba(155,63,245,0.65)' }}
        >
          {showGame ? '✕ Hide game' : '🎮 Play Breakout'}
        </button>
        <button
...
function Leaderboard({ board, compact = false }: { board: ScoreEntry[]; compact?: boolean }) {
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: compact ? 'transparent' : 'rgba(16,16,24,0.92)', border: compact ? 'none' : '1px solid rgba(155,63,245,0.4)' }}>
      <p className="font-heading font-bold text-center mb-3 tracking-[0.2em] text-sm" style={{ color: '#38E8B0' }}>HIGH SCORES — BREAKOUT</p>
